...
resource "yandex_iam_service_account" "lebedev" {
  name        = "lebedev"
...
resource "yandex_resourcemanager_folder_iam_member" "editor" {
  folder_id = "b1gmlttm66sbucm9033f"
  role      = "editor"
  member    = "serviceAccount:${yandex_iam_service_account.lebedev.id}"
}

resource "yandex_compute_instance_group" "ig-1" {
  name                = "fixed-ig-with-balancer"
  folder_id           = "b1gmlttm66sbucm9033f"
  service_account_id  = "${yandex_iam_service_account.lebedev.id}"
